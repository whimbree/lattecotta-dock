    ...
    KSharedConfig::Ptr rulesConfig, const QString &xWindowsWMClassName)
{
        ...
        if (!appId.isEmpty()) {
            ...
            if (services.isEmpty()) {
                services = KApplicationTrader::query([&appId](const KService::Ptr &service) {
                    return !service->exec().isEmpty()
                            && service->property<QString>(QStringLiteral("StartupWMClass")).compare(appId, Qt::CaseInsensitive) == 0;
                });
            ...
            if (services.isEmpty() && !xWindowsWMClassName.isEmpty()) {
                services = KApplicationTrader::query([&xWindowsWMClassName](const KService::Ptr &service) {
                    return !service->exec().isEmpty()
                            && service->property<QString>(QStringLiteral("StartupWMClass")).compare(xWindowsWMClassName, Qt::CaseInsensitive) == 0;
                });
                    ...
                    for (const QString &rule : rules) {
                        ...
                        if (match.hasMatch()) {
                            ...
                            services = KApplicationTrader::query([&rewrittenString, &serviceSearchIdentifier](const KService::Ptr &service) {
                                return !service->exec().isEmpty()
                                        && service->property<QString>(serviceSearchIdentifier).compare(rewrittenString, Qt::CaseInsensitive) == 0;
                            });
            ...
            if (!mapped.isEmpty() && services.isEmpty()) {
                services = KApplicationTrader::query([&mapped](const KService::Ptr &service) {
                    return !service->exec().isEmpty()
                            && service->desktopEntryName().compare(mapped, Qt::CaseInsensitive) == 0
                            && !service->noDisplay();
                });
            ...
            if (!mapped.isEmpty() && services.isEmpty()) {
                services = KApplicationTrader::query([&mapped](const KService::Ptr &service) {
                    return !service->exec().isEmpty()
                            && service->name().compare(mapped, Qt::CaseInsensitive) == 0
                            && !service->noDisplay();
                });
                sortServicesByMenuId(services, mapped);
            }
            ...
            if (services.isEmpty()) {
                services = KApplicationTrader::query([&appId](const KService::Ptr &service) {
                    return !service->exec().isEmpty()
                            && service->desktopEntryName().compare(appId, Qt::CaseInsensitive) == 0;
                });
                sortServicesByMenuId(services, appId);
            }
            ...
            if (services.isEmpty()) {
                services = KApplicationTrader::query([&appId](const KService::Ptr &service) {
                    return !service->exec().isEmpty()
                            && service->name().compare(appId, Qt::CaseInsensitive) == 0
                            && !service->noDisplay();
                });
                sortServicesByMenuId(services, appId);
            }
    ...
    if (services.isEmpty() || services.at(0)->desktopEntryName().isEmpty()) {
        auto matchingServices = KApplicationTrader::query([&appId](const KService::Ptr &service) {
            return !service->exec().isEmpty() && service->desktopEntryName().contains(appId, Qt::CaseInsensitive);
        });
    ...
    KSharedConfig::Ptr rulesConfig)
{
    ...
    auto queryByExec = [](const QString &exec) {
        return KApplicationTrader::query([&exec](const KService::Ptr &service) {
            return service->exec().compare(exec, Qt::CaseInsensitive) == 0;
        });
    };

    services = queryByExec(cmdLine);
    ...
    if (services.isEmpty()) {
        ...
        if (slash > 0) {
            services = queryByExec(cmdLine.mid(slash + 1));
        }
    }
