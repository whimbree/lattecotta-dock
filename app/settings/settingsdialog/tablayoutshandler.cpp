...
void TabLayouts::initLayoutTemplatesSubMenu()
{
    ...
    if (templates.rowCount() > 0) {
        ...
        connect(openTemplatesDirectory, &QAction::triggered, this, [&]() {
            KIO::highlightInFileManager({QUrl::fromLocalFile(Latte::configPath() + "/latte/templates/Dock.layout.latte")});
        });
    }
}
...
void TabLayouts::onRawLayoutDropped(const QString &rawLayout)
{
    ...
    if (importedlayout.isEmpty()) {
        showInlineMessage(i18nc("settings:dropped layout import failed","Dropped layout import <b>failed</b>..."),
                          KMessageWidget::Error,
                          true);
        return;
    }
