...
constexpr int ExpectedCoverageRelations = 1272;
...
void SettingsInventoryTest::validatesInventory()
{
    ...
    QCOMPARE(result.inventory.measured.value(QStringLiteral("candidates")).toInt(), 740);
...
void SettingsInventoryTest::rejectsCoverageRelationCountDrift()
{
    ...
    const QString errors = validateInventory(mutation).errors.join(QLatin1Char('\n'));
    QVERIFY2(errors.contains(QStringLiteral("coverage relation count is stale: expected 1272, ledger has 1273")),
             qPrintable(errors));
}
