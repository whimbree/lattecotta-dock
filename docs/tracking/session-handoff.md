...
commit: D289 (the duplicate screen-tracker spinbox connection) deleted; D296
...
at the D-Bus boundary. Remaining planned on the branch: D60 (stale Tasks
plugins.qmltypes), D272 (storagetest root QSKIP), D64 (fakepointer xkbcommon
...
one view and passes. Merged PR #139 commits `cd74a9244`, `96040fe34`,
`6f96f9408`, and `d17a81367` carry the three independent roots.
...
schema 7 already exposes the required product state, but the inherited stress
recipe does not consume it correctly. D211 records a nonexistent
`visibleGeometry` lookup that silently compares null values instead of
`currentVisibleGeometry`. D212 records the missing fixture transaction and
cleanup. D213 records a seed log that cannot be supplied as replay input.
D214 records that the ambient seed need not be a floating Panel and that the
transition, reservation, exact edit, runtime-object, and compositor-window
authorities are not asserted.

The repair is one typed acceptance boundary rather than four local guards: a
versioned immutable operation plan generated by a repository-owned fixed
algorithm, symbolic identity binding, explicit replay input, a generated
partial floating Panel fixture, complete schema-7 projections, and
transaction-safe restoration on every exit.
...
front door). Recipe 061 formerly required schema version 6 and still depends on
an externally prepared three-view fixture, so it never participated in the
schema 7 gate. PR #139 commit `1427e9b41` updates the first-read boundary to
schema 10. The deterministic fixture and registered invocation remain a
separate open task and must not be hidden inside the topology recipe.
...
FIXES. D170 (the first correction for D169 (panel shadows consumed the stable
panel and applet span) weakened end-hover shadow bounds) remained incomplete:
the complete-visual clamp proved that external paint could fit somewhere in
the canvas, not that it could fit around the requested stable solid. A 90 px
solid with a 10 px head shadow in a 100 px canvas moved five pixels when
...
the same validated bridge.

The sanitizer-backed behavioral suite covers the exact failure, swapped
asymmetry, fit-capable and clipped paint, both end clamps, full and near-full
canvases, and shadow toggles. The source guard pins horizontal left/right and
...
passing CTest entries before D171. The complete rerun passed at the
pre-D170-correction source head represented by `5461b643b`, with 105 of 105
...
preserves the stable solid while clipping external paint at that canvas. Each
portrait or landscape view supplies its own canvas, so disconnected, partially
touching, and fully touching output topologies do not enter the calculation.
...
and D170 made the stable solid authoritative while allowing external paint to
clip at the output.
...
## 2026-07-21: PR #109 dock identity isolation merged

D77 (dock duplication retains clone lineage and edit ownership) is fixed in
merged PR #109. The implementation remains
...
The commits on `main` establish relationship capability policy
(`d9ca7bcfb`), serialize settings-chrome ownership and linked-member edit
targeting (`0234aba66`), retire replica membership during destruction
(`896f8e20b`), require exact Wayland window titles (`a2a93b965`), count
ignored-window owners (`2d5184665` plus caller contract `bce41d191`), replace
stale output geometry subscriptions (`0f04cb7ef`), and preserve persistent menu
identity across runtime view recreation (`5585c708a`). The investigation and
ownership model began at `9d0a8a886`.
...
by target (`b99bbe4be`), apply the relationship policy at Corona and View
boundaries (`2695d2355`), and add the missing copyright attribution
(`08fceb456`). The branch history was rewritten to repair the commit message.
...
contract. `2e97f88b6` makes Duplicate Dock a relation-breaking snapshot from
...
`ebb517a67` moves relationship breaking into the const
...
plugin compile. The nested edit cancellation recipe (`3a7b01f25`) completed the
...
`2e97f88b6` made All Screens original 1 and linked replica 12 each create exactly
...
Commit `f31d14c49` adds all four in sorted order; the focused ratchet passes with
...
lineage. Commit `ebb517a67` centralizes the const transformation, pins its value
semantics, and checks both callers. D96 (Duplicate settings inventory still
claims linked exclusion) corrected the stale semantic row in `f755d9008`.
...
commit-message conformance issues; both were corrected during branch-history
cleanup. The final canonical gate passed at exact pre-merge head
`defaa0c7ad1a0e376937bf07f035430ecc977407` with 104/104 CTest entries, both
ratchets, 13 scene probes, three sanitizer recipes, and the deterministic output
matrix. GitHub rebased the validated source and test tree through `b6ba7ab15`
