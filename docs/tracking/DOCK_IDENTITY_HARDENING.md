...
| Rapid output, edge, alignment, and layout changes | An older reveal could clear a newer move, QWindow geometry, LayerShell output, reservation membership, and applied generation could describe different placements, or a cross-layout write could lose its recovery record | The immutable 78-operation plan for seed 127934575 contains an exact A-to-B-to-A placement probe and two real cross-layout transactions; exact schema-7 projections converge and the journal creation, commit, and retirement generations advance together |
...
plan and symbolic identity model. Seed 127934575 now drives 78 operations
through independent Duplicate Dock, explicit linked creation, two active
layouts, cross-layout and cross-output placement, all edge orientations and
semantic alignments, edit entry and exit, removal, creation after numeric ID
reuse, runtime recreation, and persistence reload. Every checkpoint validates
the complete schema-7 ownership projection, exact compositor window multiset,
nonoverlapping stable spans, maximum-depth reservation groups, settled
equivalence, and an empty durable move set. The two cross-layout operations
must advance journal creation, commit decision, and retirement together. The
accepted plan can be supplied back as replay input and must produce the same
resolved operation log. The transaction restores its pristine nested
configuration after both success and failure.
