...
| b12cce721 | 2026-07-03 | fix: add containment fallback for plasmoid inEditMode detection | CHECK | **Corroborates the REVIEW_NOTES edit-mode-entry open question.** ng found `plasmoid.userConfiguring` alone unreliable when the bridge isn't connected yet, so added `plasmoid.containment.userConfiguring` as a fallback. Our `plasmoid/main.qml:193` uses exactly ng's **pre-fix** form: `latteInEditMode || Plasmoid.userConfiguring`. Verify our plasmoid reliably detects edit mode on entry; adopt the containment fallback if not. |
